    ...
    render: function() {
        ...
        var employmentHistory = [{
            ...
            projects: [{
                ...
                desc: '将影人、影库后台整体由 jQuery 重构为 React，进行前后端分离，前端负责权限管控、API代理和页面托管。node 作为 web应用服务端，首次试水同构开发（SSR）。由于该后台对接的后台众多，在线下联调时经常因为 API接口得管理带来不少问题，因此实现了一个简单的 API Gateway，在 API Gateway 将 Java 端的各个系统模块统一注册，根据当前环境自动分发 API 调用'
            }]
        }, {
            ...
            projects: [{
                ...
                desc: "运营后台为 05 年的项目，并且从未进行技术重构，接手后在不影响业务开发的情况下将整个项目重构为 React 技术栈、前后端分离，改造为现代前端项目。建设公益类相关业务组件（项目详情、项目评价、介绍）"
            }, {
